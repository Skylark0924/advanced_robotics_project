...
class DesiredTraj:
    ...
    def get_instant_pose_circle(self, t):
        ...
        vy = 0
        vth = 3
        delta_x = (vx * math.cos(self.thetat) - vy * math.sin(self.thetat)) * dt
        delta_y = (vx * math.sin(self.thetat) + vy * math.cos(self.thetat)) * dt
        delta_th = vth * dt
        # pdb.set_trace()
        self.xt += delta_x
        self.yt += delta_y
...
class KineControl:
    ...
    def control(self):
        ...
        for t in np.arange(0, ts, dt).reshape(-1):
            ...
            # manupilablity
            W = np.append(W, np.array(np.sqrt(np.linalg.det(J @ J.T))).reshape(1, -1), axis=0)


            J_pseu = np.linalg.pinv(J)
...
if __name__ == '__main__':
    DsrTraj = DesiredTraj(x0=0, y0=0, z0=0, mode='circle', timespan=3, delta_t=0.001, r=1, num=10, shift=10)
